      ...
      case (l, atomicUpdateLocking atomicBinOp $ histOp op) of
        ...
        (Nothing, AtomicLocking f) -> do
          ...
          sComment "All locks start out unlocked" $
            groupCoverSpace [sExt64 $ kernelGroupSize constants] $ \is ->
...
compileGroupResult _ pe (TileReturns [(w, per_group_elems)] what) = do
  ...
  localOps threadOperations $
    if toInt64Exp per_group_elems == sExt64 (kernelGroupSize constants)
      then
        sWhen (offset + ltid .<. toInt64Exp w) $
          copyDWIMFix (patElemName pe) [ltid + offset] (Var what) [ltid]
      else sFor "i" (n `divUp` sExt64 (kernelGroupSize constants)) $ \i -> do
        j <- dPrimVE "j" $ sExt64 (kernelGroupSize constants) * i + ltid
